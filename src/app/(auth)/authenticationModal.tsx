...
import { ArrowBigRightIcon, X, Fingerprint } from "lucide-react";
...
const AuthenticationModal = ({ isOpen, close }: ModalProps) => {
  ...
  const [signState, setSignState] = React.useState<"login" | "register">("login");
  const [authState, setAuthState] = useState<"login" | "recover" | "otp" | "2fa">("login");
  ...
  // Handle login success from LoginForm — check if 2FA is required
  const handleLoginSuccess = (data: any) => {
    if (data.requires2FA || data.has2faEnabled) {
      setTwoFactorUserId(data.user?._id || "");
      setAuthState("2fa");
    }
  };
          ...
          {(authState === "login" || authState === "recover") && (
            <>
              ...
              <div className="mt-5 md:mt-8 flex flex-col gap-4">
                ...
                >
                  <svg width="25" height="24" viewBox="0 0 31 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M27.7569 12.5519H26.75V12.5H15.5V17.5H22.5644C21.5338 20.4106 18.7644 22.5 15.5 22.5C11.3581 22.5 8 19.1419 8 15C8 10.8581 11.3581 7.5 15.5 7.5C17.4119 7.5 19.1513 8.22125 20.4756 9.39937L24.0112 5.86375C21.7787 3.78312 18.7925 2.5 15.5 2.5C8.59688 2.5 3 8.09688 3 15C3 21.9031 8.59688 27.5 15.5 27.5C22.4031 27.5 28 21.9031 28 15C28 14.1619 27.9137 13.3438 27.7569 12.5519Z" fill="#FFC107"/>
                    <path d="M4.44116 9.18188L8.54804 12.1938C9.65929 9.4425 12.3505 7.5 15.4999 7.5C17.4118 7.5 19.1512 8.22125 20.4755 9.39937L24.0112 5.86375C21.7787 3.78312 18.7924 2.5 15.4999 2.5C10.6987 2.5 6.53491 5.21062 4.44116 9.18188Z" fill="#FF3D00"/>
                    <path d="M15.5 27.4999C18.7287 27.4999 21.6625 26.2643 23.8806 24.2549L20.0118 20.9812C18.7149 21.9681 17.1297 22.5017 15.5 22.4999C12.2487 22.4999 9.48808 20.4268 8.44808 17.5337L4.37183 20.6743C6.44058 24.7224 10.6418 27.4999 15.5 27.4999Z" fill="#4CAF50"/>
                    <path d="M27.7569 12.5519H26.75V12.5H15.5V17.5H22.5644C22.0714 18.8853 21.1833 20.0957 20.01 20.9819L20.0119 20.9806L23.8806 24.2544C23.6069 24.5031 28 21.25 28 15C28 14.1619 27.9137 13.3438 27.7569 12.5519Z" fill="#1976D2"/>
                  </svg>
                  <span className="ml-2">
