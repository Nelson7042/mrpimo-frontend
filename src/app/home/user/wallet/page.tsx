...
import { Eye, EyeOff, ChevronDown, ChevronUp, Settings, Plus, ArrowRight, CreditCard, Building2, ShoppingCart, RefreshCw, Lock, Unlock, ArrowUpRight, ArrowDownLeft, Info } from "lucide-react"
...
export default function WalletPage() {
  ...
  const escrowBalance = walletData?.wallet?.balances?.escrow || 0
  const { data: escrowTransactionsData } = useWalletTransactions(
    escrowBalance > 0 ? { type: 'escrow_hold', status: 'completed', limit: 50 } : { type: 'escrow_hold', status: 'completed', limit: 50 }
  )

  const escrowHoldTransactions = useMemo(() => {
    if (!escrowTransactionsData?.data?.transactions) return []
    return escrowTransactionsData.data.transactions.filter(
      (t: IWalletTransaction) => t.type === 'escrow_hold' && t.status === 'completed'
    )
  }, [escrowTransactionsData])
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        {/* Balance Card */}
        <div className="bg-primary text-white rounded-lg p-4 sm:p-6 mb-6">
              ...
              <div className="flex-1">
                ...
                {showEscrowDetails && (
                  <div className="mt-3 border-t border-blue-200 pt-3">
                    {escrowHoldTransactions.length > 0 ? (
                      <div className="space-y-2">
                        <p className="text-xs font-medium text-blue-800 uppercase">Orders holding escrow</p>
                        {escrowHoldTransactions.map((transaction: IWalletTransaction) => (
                          <div key={transaction.id} className="flex items-center justify-between bg-white rounded-md px-3 py-2 text-sm">
                      ...
                      <p className="text-xs text-blue-600">No active escrow hold details available.</p>
                    )}
                  </div>
                )}
              </div>
